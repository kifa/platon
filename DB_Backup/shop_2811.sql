...
(11,	'chudas@atlas.cz',	'Italská 123',	18299,	'Ostrava',	NULL)
...
ON DUPLICATE KEY UPDATE `BannerID` = VALUES(`BannerID`), `BannerType` = VALUES(`BannerType`), `BannerValue` = VALUES(`BannerValue`), `BannerLink` = VALUES(`BannerLink`);
...
ON DUPLICATE KEY UPDATE `BannerTypeID` = VALUES(`BannerTypeID`), `BannerTypeCode` = VALUES(`BannerTypeCode`), `BannerTypeName` = VALUES(`BannerTypeName`), `BannerTypeDescription` = VALUES(`BannerTypeDescription`);
...
ON DUPLICATE KEY UPDATE `CategoryID` = VALUES(`CategoryID`), `CategoryName` = VALUES(`CategoryName`), `CategorySeoName` = VALUES(`CategorySeoName`), `CategoryDescription` = VALUES(`CategoryDescription`), `CategoryStatus` = VALUES(`CategoryStatus`), `HigherCategoryID` = VALUES(`HigherCategoryID`), `CategoryPhoto` = VALUES(`CategoryPhoto`);
...
(2,	'Featured'),
(4,	'Blog')
ON DUPLICATE KEY UPDATE `CategoryStatusID` = VALUES(`CategoryStatusID`), `CategoryStatusName` = VALUES(`CategoryStatusName`);
...
ON DUPLICATE KEY UPDATE `DeliveryID` = VALUES(`DeliveryID`), `DeliveryName` = VALUES(`DeliveryName`), `DeliveryDescription` = VALUES(`DeliveryDescription`), `DeliveryPrice` = VALUES(`DeliveryPrice`), `FreeFromPrice` = VALUES(`FreeFromPrice`), `StatusID` = VALUES(`StatusID`), `HigherDelivery` = VALUES(`HigherDelivery`);
...
ON DUPLICATE KEY UPDATE `ModuleID` = VALUES(`ModuleID`), `ModuleName` = VALUES(`ModuleName`), `CompModuleName` = VALUES(`CompModuleName`), `ModuleDescription` = VALUES(`ModuleDescription`), `ModuleType` = VALUES(`ModuleType`), `StatusID` = VALUES(`StatusID`);
...
ON DUPLICATE KEY UPDATE `NotesID` = VALUES(`NotesID`), `OrderID` = VALUES(`OrderID`), `NotesDate` = VALUES(`NotesDate`), `NotesName` = VALUES(`NotesName`), `NotesDescription` = VALUES(`NotesDescription`);
...
ON DUPLICATE KEY UPDATE `OrderID` = VALUES(`OrderID`), `StatusID` = VALUES(`StatusID`), `UsersID` = VALUES(`UsersID`), `ProductsPrice` = VALUES(`ProductsPrice`), `DeliveryPaymentPrice` = VALUES(`DeliveryPaymentPrice`), `TaxPrice` = VALUES(`TaxPrice`), `PriceWithoutTax` = VALUES(`PriceWithoutTax`), `TotalPrice` = VALUES(`TotalPrice`), `DateCreated` = VALUES(`DateCreated`), `DateOfLastChange` = VALUES(`DateOfLastChange`), `DateFinished` = VALUES(`DateFinished`), `DeliveryID` = VALUES(`DeliveryID`), `PaymentID` = VALUES(`PaymentID`), `Note` = VALUES(`Note`), `IP` = VALUES(`IP`), `SessionID` = VALUES(`SessionID`), `Read` = VALUES(`Read`);
...
ON DUPLICATE KEY UPDATE `PaymentID` = VALUES(`PaymentID`), `PaymentName` = VALUES(`PaymentName`), `PaymentPrice` = VALUES(`PaymentPrice`), `StatusID` = VALUES(`StatusID`);
...
ON DUPLICATE KEY UPDATE `PhotoAlbumID` = VALUES(`PhotoAlbumID`), `PhotoAlbumName` = VALUES(`PhotoAlbumName`), `PhotoAlbumDescription` = VALUES(`PhotoAlbumDescription`), `ProductID` = VALUES(`ProductID`), `BlogID` = VALUES(`BlogID`), `StaticTextID` = VALUES(`StaticTextID`);
...
(11,	'Assam',	NULL),
(12,	'Dilmah',	NULL)
ON DUPLICATE KEY UPDATE `ProducerID` = VALUES(`ProducerID`), `ProducerName` = VALUES(`ProducerName`), `ProducerDescription` = VALUES(`ProducerDescription`);
...
(22,	'Klatch Coffee - Blue Thunder Blend',	'Klatch Coffee - Blue Thunder Blend',	NULL,	NULL,	3,	'11111',	'A strong, rich cup with characteristic smokey, sweet tones. A full bodied cup resulting from a combination of our French Roast cut with a full city roast Sumatra Blue Lintong. ',	'A strong, rich cup with characteristic smokey, sweet tones. A full bodied cup resulting from a combination of our French Roast cut with a full city roast Sumatra Blue Lintong.&nbsp;',	0,	'123456',	'122',	'rok',	9,	1007,	'0000-00-00',	'2013-09-12 21:39:35',	NULL),
...
ON DUPLICATE KEY UPDATE `ProductID` = VALUES(`ProductID`), `ProductName` = VALUES(`ProductName`), `ProductSeoName` = VALUES(`ProductSeoName`), `ProductVariants` = VALUES(`ProductVariants`), `ProductVariantName` = VALUES(`ProductVariantName`), `ProducerID` = VALUES(`ProducerID`), `ProductNumber` = VALUES(`ProductNumber`), `ProductShort` = VALUES(`ProductShort`), `ProductDescription` = VALUES(`ProductDescription`), `ProductStatusID` = VALUES(`ProductStatusID`), `ProductEAN` = VALUES(`ProductEAN`), `ProductQR` = VALUES(`ProductQR`), `ProductWarranty` = VALUES(`ProductWarranty`), `PiecesAvailable` = VALUES(`PiecesAvailable`), `CategoryID` = VALUES(`CategoryID`), `DateOfAvailable` = VALUES(`DateOfAvailable`), `ProductDateOfAdded` = VALUES(`ProductDateOfAdded`), `Video` = VALUES(`Video`);
...
(4,	'Removed',	'Not visible products')
...
(40,	'Medium',	'150'),
(41,	'Large',	'300')
ON DUPLICATE KEY UPDATE `SettingID` = VALUES(`SettingID`), `SettingName` = VALUES(`SettingName`), `Value` = VALUES(`Value`);
...
ON DUPLICATE KEY UPDATE `StaticTextID` = VALUES(`StaticTextID`), `StaticTextName` = VALUES(`StaticTextName`), `StaticTextContent` = VALUES(`StaticTextContent`), `StatusID` = VALUES(`StatusID`);
...
(2,	'Non-Active'),
(3,	'Archived')
ON DUPLICATE KEY UPDATE `StatusID` = VALUES(`StatusID`), `StatusName` = VALUES(`StatusName`);
